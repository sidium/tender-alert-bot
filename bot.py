...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS tenders (
        id TEXT PRIMARY KEY, title TEXT, price REAL, region TEXT, url TEXT, pub_date TEXT
    )''')
    c.execute('''CREATE TABLE IF NOT EXISTS users (
        user_id INTEGER PRIMARY KEY, keywords TEXT, region TEXT, max_price REAL
    )''')
    conn.commit()
    conn.close()
...
def get_new_tenders_from_rss():
    try:
        ...
        if not feed.entries:
            print("[INFO] RSS пустой или недоступен.")
            return []
        new_tenders = []
        ...
        for entry in feed.entries:
            ...
            new_tenders.append({'id': reg_num, 'title': entry.title, 'url': entry.link, 'pub_date': pub_iso})
        return new_tenders
    except Exception as e:
        print(f"[FATAL] RSS ошибка: {e}")
        return []
...
async def on_startup(dispatcher):
    print("[STARTUP] Бот запущен. Проверка каждые 15 мин...")
    asyncio.create_task(scheduler())

async def scheduler():
    while True:
        print(f"[{datetime.now(timezone.utc).isoformat()}] Бот жив. Проверка через {CHECK_INTERVAL//60} мин...")
        await check_tenders()
